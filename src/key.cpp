...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig) const {
    ...
    do {
        ...
        if (secp256k1_ecdsa_sign((const unsigned char*)&hash, (unsigned char*)&vchSig[0], &nSigLen, begin(), nonce.begin()))
...
bool CKey::SignCompact(const uint256 &hash, std::vector<unsigned char>& vchSig) const {
    ...
    do {
        nonce.MakeNewKey(true);
        if (int ret = secp256k1_ecdsa_sign_compact((const unsigned char*)&hash, &vchSig[1], begin(), (unsigned char*)&nonce, &rec))
