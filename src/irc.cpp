...
void ThreadIRCSeed()
{
    ...
    RenameThread("stakecoin-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #stakecoincoinTEST\r");
            Send(hSocket, "WHO #stakecoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #stakecoincoin\n");
            // randomly join #stakecoincoin00-#stakecoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #stakecoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #stakecoincoin%02d\r", channel_number).c_str());
        };
