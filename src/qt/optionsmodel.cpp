...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStakecoinAmount"))
        settings.setValue("nAnonymizeStakecoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeStakecoinAmount = settings.value("nAnonymizeStakecoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeStakecoinAmount"))
        SoftSetArg("-anonymizestakecoinamount", settings.value("nAnonymizeStakecoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeStakecoinAmount:
            nAnonymizeStakecoinAmount = value.toInt();
            settings.setValue("nAnonymizeStakecoinAmount", nAnonymizeStakecoinAmount);
            emit anonymizeStakecoinAmountChanged(nAnonymizeStakecoinAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
